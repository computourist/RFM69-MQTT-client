...
//	5	ACK:			read/set acknowledge message after 'SET' request (ON/OFF/READ)
//	6	toggle:			read/set toggle function on button press (ON/OFF/READ)
//	7	timer:			read/set activation timer after button press in seconds, 0 means no timer
//	8	buttonpress:		read/set flag to send a message when button pressed (ON/OFF/READ)
//
//	10	actuator:		read/set LED or relay output (ON/OFF/READ)
...
if (!mqttClient.loop()) {				// check connection MQTT server 
	mqttCon = 0;					// process MQTT subscription input in mqtt_subs routine
	digitalWrite(MQCON, LOW);
	while(mqttCon != 1){				// try to connect every 2 seconds
		mqttCon = mqttClient.connect(clientName);
		delay(2000);
	}
}
digitalWrite(MQCON, mqttCon);				// activate MQTT connection indicator
}	// end loop
...
if (respNeeded) { 					// if not succeeded in sending packets after 5 retries
	sprintf(buff_topic, "home/rfm_gw/nb/node%02d/dev00", target);	// construct MQTT topic for radio loss (dev 0)
