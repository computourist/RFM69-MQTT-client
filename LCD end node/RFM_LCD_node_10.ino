...
// x:yyyyyyyyyyy
// where x is the line number (0-1 or 0-3 depending on LCD device type) and yyyy is the text to be displayed.
//
...
// Buttons are connected between GND and D0 and D1 with pullups resistors (10k) to 3.3 Volts.
...
//	40	BTN1:			sends a message when pressed
//	41	BTN2:			sends a message when pressed
...
#define VERSION "LCD V1.0"				// this value can be queried as device 3
...
if (send40) {							// Acknowledge on 'SET'
	mes.devID = 40;
	mes.intVal = 1;						// state (integer)
	send40 = false;
	txRadio();
}
if (send41) {							// Acknowledge on 'SET'
	mes.devID = 41;
	mes.intVal = 1;						// state (integer)
	send41 = false;
	txRadio();
